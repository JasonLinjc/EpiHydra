...
threshold=0.1
experiment_name = f'{threshold}-blt_deep_cnn-cross-rela'
epochs = 50
if project_name == 'test':
    epochs = 1

batch_size=32
args = ByteLatentTransformerArgs(vocab_size=130)
model = BLT(args, lr=lr, weight_decay=weight_decay)
...
callbacks = get_callbacks(monitor='val_pr', monitor_mode='max', project_name=project_name, experiment_name=experiment_name)
trainer = Trainer(
    callbacks=callbacks,
