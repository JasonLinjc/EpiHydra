...
experiment_name = f'entropy_model-{hidden_dim}-MPRA-RC-199'
...
model = EntropyModel(hidden_dim, num_heads, max_length, output_channels=5)
...
# trainset = utils.ByteLevelTextDataset(file_path='./data/wikitext-2/train_text.csv', seq_length=max_seq_len)
# validset = utils.ByteLevelTextDataset(file_path='./data/wikitext-2/test_text.csv', seq_length=max_seq_len)
# testset = utils.ByteLevelTextDataset(file_path='./data/wikitext-2/test_text.csv', seq_length=max_seq_len)
trainset = utils.MPRADataset('./data/MPRADataset_200.txt', set_type='train', reverse_complement=True)
validset = utils.MPRADataset('./data/MPRADataset_200.txt', set_type='valid', reverse_complement=True)
testset = utils.MPRADataset('./data/MPRADataset_200.txt', set_type='train', reverse_complement=True)
...
callbacks = utils.get_callbacks(monitor='val_loss', monitor_mode='min', project_name=project_name, experiment_name=experiment_name)
