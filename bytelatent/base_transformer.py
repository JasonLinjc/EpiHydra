...
from model.transformer.diff_attention import lambda_init_fn
...
class RotaryEmbedding(torch.nn.Module):
    ...
    def __init__(self, theta: float, head_dim: int, max_seqlen = 1024):
        super().__init__()
...
class DiffAttention(nn.Module):
    def __init__(
        ...
        self.num_heads = n_heads  # // args.model_parallel_size
        self.num_kv_heads = n_heads  # args.decoder_kv_attention_heads // args.model_parallel_size if args.decoder_kv_attention_heads is not None else num_heads // args.model_parallel_size
        self.n_rep = self.num_heads // self.num_kv_heads

        self.head_dim = self.embed_dim // n_heads // 2
    ...
    def forward(
        ...
        q, k = apply_rotary_emb(q, k, 1, freq_cis[0:tgt_len])
...
class TransformerBlock(nn.Module):
    def __init__(self, args: BaseTransformerArgs, depth=0):
        super().__init__()
...
class BaseTransformer(nn.Module):
    def __init__(self, args: BaseTransformerArgs):
        ...
        self.start_depth = 0
        if 'GlobalTransformer' in str(self.__class__):
            self.start_depth = args.n_layers_local_encoder+1

        for i in range(args.n_layers):
            self.layers.append(TransformerBlock(args, i+self.start_depth))
